...
mysqli_report(MYSQLI_REPORT_ERROR | MYSQLI_REPORT_STRICT);
			...
			$sql = "SELECT distinct title_media,descr_media,date_media,keywords_media,link_media,publisher_media,type_media,text_media,path_media FROM media WHERE fk_event=$q ORDER BY date_media DESC";

$stmt = $conn->prepare($sql);
$stmt->execute();
$stmt->store_result();
$stmt->bind_result($title, $descr, $date, $key,$link,$publisher,$type,$text,$path);
					if ($stmt->num_rows > 0) {	
					while($stmt->fetch()){
		...
		if(in_array($fileType, $allowTypes)){
					...
					.$key."</p>"."<h5>Link:</h5>".'<a href="'.$link.'"target="_blank">'
					.$link.'</a>'."<h6> Publisher:".$publisher."</h6>"."<h6>Type:".$type."</h6>"."<p>"
					.$text."</p>".'<img src="'.$path.'" style="width:700px;height:500px;" alt="Broken file">'
					.'</div>'."<br>"
					."<br>"."</div>";
				}
